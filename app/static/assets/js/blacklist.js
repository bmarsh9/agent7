$(document).ready(function (){
   ...
   var table = $('#example').DataTable({
      'ajax': {
         'url': '/api/raw/dbblacklist?as_datatables=true&inc_fields=id,datatype,datavalue,enabled&limit=100'
      },
      'columnDefs': [
      {
         'targets': 0,
   ...
   $('#example tbody').on('click', 'i.tim-icons.icon-check-2, i.tim-icons.icon-button-power', function(e) {
     ...
     $.ajax({
        type: "POST",
        url: "/api/raw/dbblacklist?crud=update&filter=id,eq,"+data[0],
...
//                location.reload();
   ...
   $("button#addModal").click(function(e) {
           ...
           $.ajax({
             type: "POST",
             url: "/api/raw/dbblacklist?crud=insert",
   ...
   $("button#editModal").click(function(e) {
           ...
           $.ajax({
             type: "POST",
             url: "/api/raw/dbblacklist?crud=update&filter=id,eq,"+$('#edit-form-id').val(),
   ...
   $("button#deleteModal").click(function(e) {
           ...
           $.ajax({
             type: "POST",
             url: "/api/raw/dbblacklist?crud=delete&filter=id,eq,"+$('#delete-form-id').val(),
             contentType: 'application/json',
             success: function(result) {
               $.notify({
                 // options
