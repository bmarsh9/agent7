$(document).ready(function (){
   ...
   var table = $('#example').DataTable({
      'ajax': {
         'url': '/api/agent/data/users?as_datatables=true&inc_fields=id,email,access,active'
      },
      'columnDefs': [
      {
         'targets': 0,
   ...
   $('#example tbody').on('click', 'i.tim-icons.icon-settings', function(e) {
     ...
     jQuery('#editModal').on('shown.bs.modal', function (e) {
          ...
          $(e.currentTarget).find('input[name="dataAccess"]').val(data[2]);
          $(e.currentTarget).find('input[name="dataActive"]').val(data[3]);
     });
   });
   // Delete
...
//                location.reload();
   ...
   $("button#addModal").click(function(e) {
           ...
           $.ajax({
             ...
             data : JSON.stringify({"email":$('#add-form-email').val(),"password":$('#add-form-password').val(),"access":$('#add-form-access').val(),"active":$('#add-form-active').val()}),
             contentType: 'application/json',
             success: function(result) {
               $.notify({
                 // options
   ...
   $("button#editModal").click(function(e) {
           ...
           $.ajax({
             ...
             data : JSON.stringify({"email":$('#edit-form-email').val(),"access":$('#edit-form-access').val(),"active":$('#edit-form-active').val()}),
             contentType: 'application/json',
             success: function(result) {
               $.notify({
                 // options
