...
from app.models import get_TableSchema
...
class DynamicQuery():
    ...
    .Example:
        ...
        query.generate().all()
    '''
    def __init__(self, model, request_args=[], filter=[], groupby=[], orderby=(), distinct=None, inc_fields=[], exc_fields=[],visible=[],
        data={}, qjson={},getfirst=False, getcount=False, as_query=False, as_json=False,as_object=True, as_datatables=False, as_chartjs=False, as_schema=False, orlist=None,concat=None,crud=None, limit=10, reset_count=False,**kwargs):
        '''
...
#            self.filter = manually set
    ...
    def to_datatables(self,data):
        ...
        .data -> [{},{}]
        '''
        data_dict = {"draw":0,"data": [],"count":0,"columns":[],"hide":[]}
        if not isinstance(data,list):
            data = [data]
        for record in data:
            data_dict["count"] += 1
