...
openssl req -new -key server.key -subj "/C=US/ST=VA/O=Fortalice" -reqexts SAN -config <(cat /etc/ssl/openssl.cnf <(printf "[SAN]\nsubjectAltName=IP:$SERVER_HOSTNAME")) -passin pass:$PASSWORD -out server.csr
echo 2
...
openssl req -new -key client.key -subj "/C=US/ST=VA/O=Fortalice" -reqexts SAN -config <(cat /etc/ssl/openssl.cnf <(printf "[SAN]\nsubjectAltName=IP:$CLIENT_HOSTNAME")) -passin pass:$PASSWORD -out client.csr
echo 4
